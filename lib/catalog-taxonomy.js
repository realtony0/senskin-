...
const FALLBACK_IMAGE_BY_ID = new Map(
  INITIAL_PRODUCTS.filter((product) => product?.i).map((product) => [Number(product.id), product.i]),
);

const FALLBACK_IMAGE_BY_NAME = new Map(
  INITIAL_PRODUCTS.filter((product) => product?.n && product?.i).map((product) => [
    String(product.n).trim().toLowerCase(),
    product.i,
  ]),
);
